...
import ray
from symbolic_safe_rl.safety_gym_utils import (
    VisionWrapper,
    SymMapCenterNet,
    State,
    SymMapOracle,
...
def run_polopt_agent(
    ...
    class RemoteEnv:
        ...
        def step(
            ...
            eps = 1e-10
            pre_sum = -0.5 * (
                ((action - mu) / (std + eps)) ** 2 + 2 * log_std + np.log(2 * np.pi)
            )
            log_p = pre_sum.sum()
    ...
    if exp:
        if "Point" in env_name:
            ...
            assert False
        task = (
            env_name.replace("-v0", "").replace("Safexp-", "").replace(robot_type, "")
        )
    ...
    if agent.learn_penalty:
        ...
        train_penalty = tf.train.AdamOptimizer(learning_rate=penalty_lr).minimize(
            penalty_loss
        )
    ...
    def update():
        ...
        for j in range(splits):
            ...
            measures = dict(
                LossPi=pi_loss, SurrCost=surr_cost, LossV=v_loss, Entropy=ent
            )
            if not (agent.reward_penalized):
            ...
            for k in post_update_measures:
                if k in pre_update_measures:
                    deltas["Delta" + k] = (
                        post_update_measures[k] - pre_update_measures[k]
                    )
    ...
    for epoch in range_(epochs):
        ...
        for t in range_(local_steps_per_epoch):
            ...
            pi_info_t = [
                {"mu": mu[i : i + 1], "log_std": log_std} for i in range(n_envs)
            ]
            ...
            o2s, rs, ds, infos, actions, logps, visual_os = zip(
                *ray.get([env.step.remote(*arg) for env, arg in zip(envs, args)])
            )
            ...
            if agent.reward_penalized:
                ...
                for i, buf in enumerate(bufs):
                    buf.store(
                        os[i], a[i], r_totals[i], v_t[i], 0, 0, logp_t[i], pi_info_t[i]
                    )
            else:
                for i, buf in enumerate(bufs):
                    buf.store(
            ...
            for i, buf in enumerate(bufs):
                ...
                if terminal or (t == local_steps_per_epoch - 1):
                    ...
                    if d and not (ep_len == max_ep_len):
                        ...
                        if visual_obs:
                            o = np.array(
                                Image.fromarray((os[i] * 255).astype(np.uint8)).resize(
                                    (IMG_RESIZE, IMG_RESIZE), resample=4
                                )
                            )
                            print(
